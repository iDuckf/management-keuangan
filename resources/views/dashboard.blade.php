<x-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-gray-900 rounded-lg py-3 font-bold">
            <h1 class="text-center">Dashboard Filter</h1>
            <div class="flex justify-center items-center pt-3">
                <form method="GET" action="{{ route('dashboard') }}">
                    <label for="filrerYear">Year : </label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-gray-900 p-6 rounded-xl border border-gray-800">
                ...
                <p class="text-3xl font-bold text-emerald-400">Rp {{ number_format($totalIncomesMonth, 0, ',', '.') }}
                </p>
            ...
            <div class="bg-gray-900 p-6 rounded-xl border border-gray-800">
                ...
                <p class="text-3xl font-bold text-red-400">Rp {{ number_format($totalExpensesMonth, 0, ',', '.') }}</p>
                <p class="text-xs text-gray-500 mt-2">{{ $totalExpensesEntries }} entries</p>
            </div>
            <div class="bg-gray-900 p-6 rounded-xl border border-gray-800">
                <h3 class="text-gray-400 text-sm">Total Balance</h3>
                <p class="text-3xl font-bold text-blue-400">Rp {{ number_format($totalBalances, 0, ',', '.') }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 font-bold">
            ...
            <div class="bg-gray-900 rounded-lg p-4 lg:col-span-2">
                <h2 class="text-center pb-3 text-white">Income VS Expense Monthly</h2>
                <canvas id="financialChart"></canvas>
            </div>

            <!-- Donut Chart (Diperkecil melalui Flexbox & Padding) -->
            <div class="bg-gray-900 rounded-lg p-4">
                <h2 class="text-center pb-3 text-white w-full">Category Expense</h2>
                <!-- Container khusus untuk mengunci ukuran maksimal bulatan donat -->
        ...
        <div class="bg-gray-900 rounded-2xl border border-gray-800 p-6">
            <h2 class="text-center pb-4 text-white font-bold">Balances Summary</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @php
                    $typeConfig = [
                        'cash' => [
                ...
                @foreach ($typeConfig as $tipe => $config)
                    ...
                    @endphp
                    <div
                        class="bg-gradient-to-br {{ $config['gradient'] }} rounded-xl p-5 border {{ $config['border'] }}">
                        <div class="flex items-center gap-3">
                            <div class="p-2.5 rounded-lg {{ $config['iconBg'] }}">
                                @if ($tipe === 'cash')
                                    <svg class="w-6 h-6 {{ $config['iconText'] }}" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                            ...
                            <div>
                                ...
                                <p class="text-xl font-bold {{ $config['amountText'] }}">Rp
        ...
        <div>
            <h1 class="text-center pb-4 text-white font-bold">Newest Incomes and Expenses</h1>
                        ...
                        <tbody>
                            @forelse ($newestIncomes as $income)
                                <tr class="hover:bg-gray-800/50 transition duration-150">
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium">{{ Str::limit($income->source, 10) }}
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium text-emerald-400">Rp.
                                        {{ number_format($income->amount, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-400">{{ $income->date->format('j F Y') }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <span
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-8 text-center text-gray-500 text-sm">Belum Ada
                    ...
                    <table class="w-full">
                        ...
                        <tbody>
                            @forelse ($newestExpenses as $expense)
                                <tr class="hover:bg-gray-800/50 transition duration-150">
                                    <td class="px-6 py-4 text-sm text-gray-400">{{ $loop->iteration }}</td>
                                    <td class="px-6 py-4 text-sm font-medium">{{ Str::limit($expense->title, 10) }}
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium text-red-400">Rp.
                                        {{ number_format($expense->amount, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-400">{{ $expense->date->format('j F Y') }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <span
            ...
            new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            mode: 'index',
                            intersect: false // Saat kursor diarahkan, langsung munculkan tooltip untuk kedua batang sekaligus
                        },
                    },
                    scales: {
                        y: {
