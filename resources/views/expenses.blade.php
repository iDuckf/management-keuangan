    ...
    <div class="space-y-4 sm:space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
                ...
                <p class="text-gray-400 mt-1 text-sm">Track your spending</p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6">
            <div class="bg-gray-900 rounded-2xl p-4 sm:p-6 border border-gray-800">
                <div class="flex items-center gap-3">
                    <div class="p-2.5 sm:p-3 bg-red-600/10 rounded-xl">
                    ...
                    <div>
                        <p class="text-gray-400 text-xs sm:text-sm">Total Expenses</p>
                        <p class="text-lg sm:text-xl font-bold mt-0.5">Rp. {{ number_format($totalExpenses, 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
            <div class="bg-gray-900 rounded-2xl p-4 sm:p-6 border border-gray-800">
                <div class="flex items-center gap-3">
                    <div class="p-2.5 sm:p-3 bg-orange-600/10 rounded-xl">
                    ...
                    <tbody class="divide-y divide-gray-800">
                        @forelse ($expenses as $expense)
                            <tr class="hover:bg-gray-800/50 transition duration-150">
                                ...
                                <td class="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-medium">{{ $expense->title }}</td>
                                <td class="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-medium text-red-400">Rp.
                                    {{ number_format($expense->amount, 0, ',', '.') }}</td>
                                <td class="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-400">{{ $expense->date->format('j F Y') }}</td>
    ...
    <div id="addExpenseModal" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-3 sm:p-4">
            <div class="bg-gray-900 rounded-2xl w-full max-w-lg border border-gray-800 shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form class="p-4 sm:p-6 space-y-4" method="POST" action="{{ route('expense-save') }}">
                    @csrf
                    @method('POST')
    ...
    <div id="editExpenseModal" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-3 sm:p-4">
            <div class="bg-gray-900 rounded-2xl w-full max-w-lg border border-gray-800 shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form id="editExpenseForm" class="p-4 sm:p-6 space-y-4" method="POST" action="">
                    @csrf
                    @method('PUT')
